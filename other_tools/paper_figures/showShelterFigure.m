...
function [model_2D_l_1, model_2D_r_1] = showShelterFigure(img_to_be_shown, estimated_pose, heatmap, center, scale, camera, model)
    
    img_crop{1} = cropImage(img_to_be_shown{1},center{1},scale{1});
    img_crop{2} = cropImage(img_to_be_shown{2},center{2},scale{2});
    
    figure_position{1} = [0   1/2  1/2 1/2];
    figure_position{2} = [1/2 1/2 1/2 1/2];
    figure_position{3} = [0   0  1/2 1/2];
    figure_position{4} = [1/2 0 1/2 1/2];
    ...
    % left figure
    figure('position',[300,300,2*300,2*300]);
    ...
    % right figure
    subplot('position',figure_position{4}); imshow(img_crop{1}); hold on;
    drawFrame(model_2D_l_2(1,:), model_2D_l_2(2,:), 2);
    plot(model_2D_l_2(1,:), model_2D_l_2(2,:), '.', 'markersize', 40);
    
    subplot('position',figure_position{3}); imshow(img_crop{2}); hold on;
